...
- `Flicker Monitoring and Control` opens `FlickerControlWindow.xaml` and contains `Flickering Monitor`, `Flicker Injection Control`, and `Info`.
...
The Flicker window is implemented as the XAML window `FlickerControlWindow.xaml`. Its current fixed size is `620 x 580` pixels and it contains the Flickering Monitor, Flicker Injection Control, and Info sections. The separate `AvtpLvdsSimulationControlWindow.xaml` contains only the LVDS Simulation Control section.
