...
WPF application (.NET 8) for real-time visualization and comparison of 8-bit grayscale LED matrix frames from automotive ECU systems (Aurix TC397).
...
| **Default Endpoint** | `http://127.0.0.1:8420/api/v1/health` |
| **Commands** | Ping, Start/Stop/Pause/Resume, SetComparisonSettings, GetComparisonStats, GetFrameSnapshot |
...
# client.headers["X-Api-Key"] = "your-key"  # only for remote

# Health check
r = client.get("http://127.0.0.1:8420/api/v1/health")
print(r.json())  # {"ok": true, "utc": "..."}

# Start simulation
r = client.post("http://127.0.0.1:8420/api/v1/command",
    json={"command": "StartSimulation", "payload": {"fps": 100}})
print(r.json())
```
...
See [docs/API.md](docs/API.md) for complete endpoint reference, authentication details, error codes, and SDK examples.
