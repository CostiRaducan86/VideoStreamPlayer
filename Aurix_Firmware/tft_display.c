...
// Modern Consolas (current):
//#include "tft_font_modern.h"
...
/* Active font table pointer — defaults to bold, switchable at runtime */
static const uint16 *s_fontTable = tft_font_table_bold;
static uint8 s_fontBold = 1u;
...
static void qspi0_init(void)
{
    ...
    QSPI0_GLOBALCON.U = (1u << IFX_QSPI_GLOBALCON_RESETS_OFF)
                       | (1u << IFX_QSPI_GLOBALCON_CLKSEL_OFF)  
...
{
    s_fontBold = (bold != 0u) ? 1u : 0u;
    s_fontTable = (s_fontBold != 0u) ? tft_font_table_bold : tft_font_table_regular;
}
...
void tft_draw_char(uint16 row, uint16 col, char c)
{
    ...
    glyph = &s_fontTable[(c - 0x20) * TFT_CHAR_HEIGHT];
    x = row;
