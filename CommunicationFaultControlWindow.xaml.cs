...
public partial class CommunicationFaultControlWindow : Window
{
    ...
    private void RefreshUi()
    {
        ...
        SetFaultInfoLines(available, active, canAvailable, canActive);
    }
    ...
    private void SetFaultInfoLines(bool avtpAvailable, bool avtpActive, bool canAvailable, bool canActive)
    {
        if (avtpActive || canActive)
        {
            string activeInfo = avtpActive
        ...
        string constraintInfo = canAvailable
            ? GetCanUartConstraintInfo().Constraints
            : "CAN-UART controls are unavailable.";

        Brush infoBrush = avtpActive || canActive ? new SolidColorBrush(Colors.DarkRed) : new SolidColorBrush(Colors.Gray);
